...
class ModelCache {
  ...
  async get(url: IDBValidKey | IDBKeyRange) {
    await this.init();
  ...
  async set(url: any, data: ArrayBuffer) {
    await this.init();
  ...
  async delete(url: IDBValidKey | IDBKeyRange) {
    await this.init();
...
export async function cachedFetch(
  ...
  let cacheKey: IDBValidKey | IDBKeyRange;
  if (
    typeof url === "string" ||
    typeof url === "number" ||
    url instanceof Date
    ...
    cacheKey = JSON.stringify(
      Array.from(new Uint8Array(url instanceof ArrayBuffer ? url : url.buffer))
    );
  ...
  if (cachedData) {
    return new Response(cachedData as ArrayBuffer);
  }
