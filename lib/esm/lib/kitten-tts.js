...
export class KittenTTS {
    ...
    static async from_pretrained(model_path, options = {}) {
        try {
            ...
            const voicesResponse = await cachedFetch(`https://raw.githubusercontent.com/rahulsushilsharma/tts-pipelines/refs/heads/main/public/tts-model/voices_kitten.json`);
    ...
    async loadTokenizer() {
        if (!this.tokenizer) {
            try {
                const { cachedFetch } = await import("../utils/model-cache.js");
                const response = await cachedFetch(`https://raw.githubusercontent.com/rahulsushilsharma/tts-pipelines/refs/heads/main/public/tts-model/tokenizer.json`);
                    ...
                    if (this.session && this.voiceEmbeddings[voice]) {
                        try {
                            ...
                            if (audioData.length > 0 && isNaN(Number(audioData[0]))) {
                                ...
                                if (!this.wasmSession) {
                                    const ort = await import("onnxruntime-web");
                                    this.wasmSession = await ort.InferenceSession.create(`https://huggingface.co/onnx-community/kitten-tts-nano-0.1-ONNX/resolve/main/onnx/model_quantized.onnx`, {
