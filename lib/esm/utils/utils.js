...
export function normalizePeak(f32, target = 0.9) {
    if (!f32?.length)
        return;
    let max = 1e-9;
...
export function trimSilence(f32, thresh = 0.002, minSamples = 480) {
...
export async function saveAudio(blob, path = "./output.wav") {
    if (isBrowser()) {
